...
@synthesize pathToDynamicallyGeneratedNamesGrammar;
@synthesize pathToDynamicallyGeneratedNamesDictionary;
...
- (void)viewDidLoad {
    ...
    //
    NSArray *namesLanguageArray = [[NSArray alloc] initWithArray:[NSArray arrayWithObjects: // All capital letters.
                                                                  @"ANTHONY",
                                                                  @"BRIAN",
                                                                  @"LAUREN",
                                                                  @"SHAWN KIRCHER",
    ...
    //languageModelGenerator2.verboseLanguageModelGenerator = TRUE; // Uncomment me for verbose debug output
    
    //
    NSError *error2 = [languageModelGenerator2 generateLanguageModelFromArray:namesLanguageArray withFilesNamed:@"OpenEarsDynamicNamesGrammar" forAcousticModelAtPath:[AcousticModel pathToModel:@"AcousticModelEnglish"]];
    
	NSDictionary *dynamicLanguageGenerationNamesResultsDictionary = nil;
	if([error2 code] != noErr) {
		NSLog(@"Dynamic language generator reported error %@", [error2 description]);
	} else {
		dynamicLanguageGenerationNamesResultsDictionary = [error2 userInfo];
        //
        NSString *lmNameFile = [dynamicLanguageGenerationNamesResultsDictionary objectForKey:@"LMFile"];
        NSString *nameDictionaryFile = [dynamicLanguageGenerationNamesResultsDictionary objectForKey:@"DictionaryFile"];
        NSString *lmNamePath = [dynamicLanguageGenerationNamesResultsDictionary objectForKey:@"LMPath"];
        NSString *nameDictionaryPath = [dynamicLanguageGenerationNamesResultsDictionary objectForKey:@"DictionaryPath"];
        //
        NSLog(@"Dynamic Names language generator completed successfully, you can find your new files %@\n and \n%@\n at the paths \n%@ \nand \n%@", lmNameFile,nameDictionaryFile,lmNamePath,nameDictionaryPath);
        
        // pathToDynamicallyGeneratedGrammar/Dictionary aren't OpenEars things, they are just the way I'm controlling being able to switch between the grammars in this sample app.
        self.pathToDynamicallyGeneratedNamesGrammar = lmNamePath; // We'll set our new .languagemodel file to be the one to get switched to when the words "CHANGE MODEL" are recognized.
        self.pathToDynamicallyGeneratedNamesDictionary = nameDictionaryPath; // We'll set our new dictionary to be the one to get switched to when the words "CHANGE MODEL" are recognized.
	...
	if(dynamicLanguageGenerationNamesResultsDictionary) {
